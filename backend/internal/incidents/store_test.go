...
func TestSaveInvestigationResultAndFailure(t *testing.T) {
    s := NewStore()

    seed := NewIncident("", "dev1", "svc", "stopped", SeverityMedium, "reason")
    inc, _ := s.CreateOrGetActive("key1", seed)

    recs := []RecommendedAction{{ActionID: "restart_service", Target: "svc", Description: "reason"}}

    updated, err := s.SaveInvestigationResult(inc.IncidentID, "cause", 0.7, recs, []string{"step1"}, "sum", time.Now().UTC(), "trace-1", "completed")
    if err != nil {
        t.Fatalf("save investigation result failed: %v", err)
    }
    if updated.ProbableCause != "cause" {
        t.Fatalf("expected probable cause saved")
    }

    // Save failure
    updated2, err := s.SaveInvestigationFailure(inc.IncidentID, "timeout", "deadline exceeded", "trace-2")
    if err != nil {
        t.Fatalf("save investigation failure failed: %v", err)
    }
    if updated2.InvestigationStatus != "timeout" {
        t.Fatalf("expected investigation status updated")
    }
}
...
func TestStore_UpdateState_ResolvePathClearsActiveIndex(t *testing.T) {
    ...
    if updated.Reason != "triage started" {
        t.Fatalf("reason: got %q, want %q", updated.Reason, "triage started")
    }
